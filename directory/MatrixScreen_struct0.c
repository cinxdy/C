...
int t = 0;
int screenWidth = 0 ;
int screenHeight = 0 ; // master clock
...
*/
	...
	screenHeight = getWindowHeight() - 2;
	...
	for(col = 0; col < screenWidth; col++){
		...
		bar[col].prevLength = bar[col].length;
	}
...
void MoveBar(struct Bar *pBar){
	...
	// erase previous location
	if(pBar->y - pBar->length >= 1){
		gotoxy(col + 1, pBar->y - pBar->length);
		putchar(' ');
	}
				
	if(pBar->y <= pBar->prevLength){
		gotoxy(col + 1, screenHeight + pBar->y - pBar->prevLength);	// wrapped
		putchar(' ');
	}
	...
	// check if y reached the last screenHeight
	if(pBar->y > screenHeight){
		pBar->y = 1;
		pBar->delay = rand() % 10 + 1;	// reset delay and length
					
		pBar->prevLength = pBar->length;
		pBar->length = rand() % 5 + 3;
	}
